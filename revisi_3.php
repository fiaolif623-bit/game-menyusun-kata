...
<html lang="id">
  ...
  <script>
    ...
    const levels = {
      1: {
        name: "Mudah",
        questions: [
          { word: "IKAN", category: "hewan", hint: "Hidup di air", question: "Hewan yang hidup di air dan bernapas dengan insang?" },
          { word: "APEL", category: "buah", hint: "Buah berwarna merah", question: "Buah yang sering berwarna merah atau hijau, sering dimakan untuk kesehatan?" },
          { word: "BALI", category: "kota", hint: "Pulau wisata", question: "Pulau wisata terkenal di Indonesia dengan pantai indah?" },
          { word: "KUDA", category: "hewan", hint: "Hewan berkaki empat", question: "Hewan berkaki empat yang bisa ditunggangi?" },
          { word: "NANAS", category: "buah", hint: "Buah berduri", question: "Buah tropis yang memiliki kulit berduri dan daging berwarna kuning?" }
        ]
      },
      2: {
        name: "Sedang",
        questions: [
          { word: "KUCING", category: "hewan", hint: "Hewan peliharaan yang suka ikan", question: "Hewan peliharaan yang suka mengeong dan berburu tikus?" },
          { word: "JERUK", category: "buah", hint: "Buah kaya vitamin C", question: "Buah bulat berwarna oranye yang kaya akan vitamin C?" },
          { word: "MEDAN", category: "kota", hint: "Kota di Sumatera Utara", question: "Kota terbesar di Sumatera Utara yang terkenal dengan kuliner?" },
          { word: "BURUNG", category: "hewan", hint: "Hewan yang bisa terbang", question: "Hewan yang memiliki sayap dan bisa terbang di udara?" },
          { word: "PISANG", category: "buah", hint: "Buah favorit monyet", question: "Buah panjang berwarna kuning yang menjadi makanan favorit monyet?" }
        ]
      },
      3: {
        name: "Sulit",
        questions: [
          { word: "KELINCI", category: "hewan", hint: "Hewan dengan telinga panjang", question: "Hewan berbulu lembut dengan telinga panjang yang suka makan wortel?" },
          { word: "SEMANGKA", category: "buah", hint: "Buah besar berair", question: "Buah besar berair dengan daging merah dan biji hitam?" },
          { word: "BANDUNG", category: "kota", hint: "Kota kembang", question: "Kota di Jawa Barat yang dijuluki sebagai Kota Kembang?" },
          { word: "DURIAN", category: "buah", hint: "Raja buah yang berbau", question: "Buah berduri dengan aroma kuat yang dijuluki Raja Buah?" },
          { word: "SURABAYA", category: "kota", hint: "Kota pahlawan", question: "Kota terbesar kedua di Indonesia yang dijuluki Kota Pahlawan?" }
        ]
      },
      4: {
        name: "Sangat Sulit",
        questions: [
          { word: "YOGYAKARTA", category: "kota", hint: "Kota pelajar dan budaya", question: "Daerah Istimewa yang terkenal dengan Candi Borobudur dan Keraton?" },
          { word: "SEMARANG", category: "kota", hint: "Kota Lumpia", question: "Ibu kota Jawa Tengah yang terkenal dengan kuliner lumpia?" },
          { word: "JAKARTA", category: "kota", hint: "Ibu kota Indonesia", question: "Ibu kota negara Indonesia yang merupakan pusat pemerintahan?" }
        ]
      }
    };
    ...
    function loadNewWord() {
      ...
      if (currentQuestionIndex >= levelData.questions.length) {
        ...
        if (currentLevel < 4) {
          messageEl.textContent = `🏆 Level ${currentLevel} Selesai! Skor: ${totalScore}. Pindah ke Level ${currentLevel + 1}...`;
          messageEl.className = 'message correct';
          document.getElementById('next-button').style.display = 'none';
          document.getElementById('check-button').style.display = 'none';
          document.getElementById('hint-button').style.display = 'none';
          
          // Pindah ke level selanjutnya setelah 3 detik
          setTimeout(() => {
            currentLevel++;
            currentQuestionIndex = 0;
            loadNewWord();
          ...
          // Semua level selesai
          messageEl.textContent = `🎉 Selamat! Semua Level Selesai! Total Skor: ${totalScore}. Klik "Kembali" untuk main lagi.`;
          messageEl.className = 'message correct';
          document.getElementById('next-button').style.display = 'none';
          document.getElementById('check-button').style.display = 'none';
          document.getElementById('hint-button').style.display = 'none';
    ...
    async function onConfigChange(config) {
      ...
      if (categoryBadge) {
        categoryBadge.style.background = config.accent_color || defaultConfig.accent_color;
      }
      
      const scrambledWord = document.querySelector('.scrambled-word');
      if (scrambledWord) {
        scrambledWord.style.color = config.secondary_color || defaultConfig.secondary_color;
      }
      
      const questionBox = document.querySelector('.question-box');
      if (questionBox) {
        questionBox.style.borderColor = config.primary_color || defaultConfig.primary_color;
      }
      
      const questionLabel = document.querySelector('.question-label');
      if (questionLabel) {
        questionLabel.style.color = config.primary_color || defaultConfig.primary_color;
        questionLabel.style.fontFamily = `${customFont}, ${baseFontStack}`;
        questionLabel.style.fontSize = `${baseSize * 1.1}px`;
      }
      
      const questionText = document.querySelector('.question-text');
      if (questionText) {
        questionText.style.color = config.text_color || defaultConfig.text_color;
        questionText.style.fontFamily = `${customFont}, ${baseFontStack}`;
        questionText.style.fontSize = `${baseSize * 1.3}px`;
      }
      
      const buttons = document.querySelectorAll('button:not(.hint-button):not(.next-button):not(.back-button)');
      buttons.forEach(btn => {
      ...
      levelCards.forEach(card => {
        card.style.background = `linear-gradient(135deg, ${config.primary_color || defaultConfig.primary_color}, ${config.secondary_color || defaultConfig.secondary_color})`;
      });
    }
 ...
 <script>(function(){function c(){var b=a.contentDocument||a.contentWindow.document;if(b){var d=b.createElement('script');d.innerHTML="window.__CF$cv$params={r:'99fe2176836fb5e4',t:'MTc2MzM3MTAwMS4wMDAwMDA='};var a=document.createElement('script');a.nonce='';a.src='/cdn-cgi/challenge-platform/scripts/jsd/main.js';document.getElementsByTagName('head')[0].appendChild(a);";b.getElementsByTagName('head')[0].appendChild(d)}}if(document.body){var a=document.createElement('iframe');a.height=1;a.width=1;a.style.position='absolute';a.style.top=0;a.style.left=0;a.style.border='none';a.style.visibility='hidden';document.body.appendChild(a);if('loading'!==document.readyState)c();else if(window.addEventListener)document.addEventListener('DOMContentLoaded',c);else{var e=document.onreadystatechange||function(){};document.onreadystatechange=function(b){e(b);'loading'!==document.readyState&&(document.onreadystatechange=e,c())}}}})();</script></body>
</html>